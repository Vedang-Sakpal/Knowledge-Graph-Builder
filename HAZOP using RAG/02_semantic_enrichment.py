#UPDATED: This script now correctly handles nested property objects from the LLM.
...
class SemanticEnricher:
    ...
    def extract_info_with_llm(self, context, prompt_template):
        print("Contacting Gemini API for information extraction...")
        ...
        with self.driver.session() as session:
            ...
            if 'chemical_data' in extracted_data:
                for chem in extracted_data.get('chemical_data', []):
                    ...
                    if name:
                        # --- START OF FIX ---
                        # Flatten the properties dictionary to handle nested objects from the LLM.
                        all_props = {'name': name}
...
def main():
    enricher = SemanticEnricher(config.NEO4J_URI, config.NEO4J_USERNAME, config.NEO4J_PASSWORD, config.GEMINI_API_KEY)
